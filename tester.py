    ...
    for i, line in enumerate(lines):
        ...
        for j, coordinate in enumerate(coordinates):
            ...
            if (coordinate - expected).copy_abs() > PERMITTED_DELTA:
                print_red(
                    f"failure: incorrect output. expected {expected}, got {coordinate}",
                    prefix_msg,
                )
                return False
    ...
    if TEST_PYTHON_MEMORY:
        ...
        with make_stub_file(test_data.X) as tmpfile:
            for goal, target_matrix in goals:
                ...
                if test_goal(
                    ProgramType.PYTHON, goal, tmpfile.name, True, target_matrix
                ):
                    print_green("success")
                else:
        ...
        for trial in range(1, TRIALS_VALGRIND_PY_SYMNMF + 1):
            ...
            with make_stub_file(test_data.X) as tmpfile:
                if test_goal(
                    ProgramType.PYTHON, "symnmf", tmpfile.name, True, target_H, k
                ):
                    successes += 1
